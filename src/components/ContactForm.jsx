...
const ContactForm = () => {
  ...
  const inputClass = (field) =>
    `w-full bg-white/10 px-4 py-3.5 rounded-xl border outline-none transition-all text-sm md:text-base font-medium text-right text-white placeholder:text-white/40 ${
      errors[field]
        ? 'border-[#F7444E] ring-1 ring-[#F7444E]/50'
        : 'border-white/20 focus:border-[#78BCC4]'
    }`;
  ...
  return (
    <form onSubmit={handleSubmit} className="text-right space-y-4">
      ...
      <button
        type="submit"
        className="w-full bg-[#F7444E] hover:bg-[#de3d46] text-white px-6 py-4 rounded-xl font-bold text-base shadow-md transition-all flex items-center justify-center gap-2"
      >
